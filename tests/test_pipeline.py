...
def _write_two_record_genbank(path: Path) -> Path:
    """Write a minimal two-record GenBank, each record carrying one CDS."""
    ...
    records = []
    for name in ("NC_ONE01", "NC_TWO01"):
        record = SeqRecord(
            Seq("ATGCTAGCTA" * 12),
            id=name,
            name=name,
            description=f"synthetic contig {name}",
        ...
        record.features.append(
            SeqFeature(
                ...
                type="CDS",
                qualifiers={"locus_tag": [f"{name}_0001"], "product": ["hypothetical"]},
            )
        )
        records.append(record)
    write(records, str(path), "genbank")
    return path


def test_tpp_reference_filename_in_chrom_header_is_not_used_as_the_contig(tmp_path: Path):
    """TPP writes the reference FASTA basename into chrom=, never the accession.

    Trusting it silently detached the counts from the candidate-site table, so
    every count was replaced by zero when unobserved sites were filled in.
    """
    ...
    def fake_runner(command: tuple[str, ...]) -> None:
        if "-output" in command:
            prefix = Path(command[command.index("-output") + 1])
            # Exactly what TPP emits: chrom= is basename("reference.fasta").
            prefix.with_suffix(".wig").write_text(
                f"# Generated by tpp from raw_R1.fastq.gz and \n"
                f"variableStep chrom=reference\n{hit} 37\n",
                encoding="utf-8",
            )
    ...
    assert {row.contig for row in result.counts} == {"NC_TEST01"}
    assert next(row for row in result.counts if row.position == hit).read_count == 37


def test_multi_contig_tpp_run_keeps_each_replicon_separate(tmp_path: Path):
    """Both replicons declare the same chrom=, so only the filename tells them apart."""

    reference = _write_two_record_genbank(tmp_path / "two_contigs.gbk")

    def fake_runner(command: tuple[str, ...]) -> None:
        if "-output" not in command:
            return
        prefix = Path(command[command.index("-output") + 1])
        # TPP appends the replicon id to the filename, and writes a non-standard
        # 'chrom=<ref>, replicon=<0-based index>' header into every file.
        for index, contig in enumerate(("NC_ONE01", "NC_TWO01")):
            prefix.with_name(f"{prefix.name}_{contig}.wig").write_text(
                f"variableStep chrom=reference, replicon={index}\n"
                f"10 {5 + index}\n",
                encoding="utf-8",
            )
    ...
    result = run_pipeline(
        ...
        PipelineConfig(
            ...
            run_fastp=False,
            include_unobserved_sites=False,
            tpp_replicon_ids=("NC_ONE01", "NC_TWO01"),
        ),
        runner=fake_runner,
    )

    # Previously both files parsed as contig 'reference,' and position 10 summed to 11.
    assert result.counts == (
        InsertionCount("NC_ONE01", 10, 5.0),
        InsertionCount("NC_TWO01", 10, 6.0),
    )


def test_multi_contig_reference_without_replicon_ids_is_rejected_before_running_tpp(
    tmp_path: Path,
):
    reference = _write_two_record_genbank(tmp_path / "two_contigs.gbk")
    invoked: list[tuple[str, ...]] = []

    with pytest.raises(PipelineError, match="tpp-replicon-ids"):
        run_pipeline(
            reference,
            ReadPair(Path("raw_R1.fastq.gz"), sample_id="synthetic"),
            PipelineConfig(
                output_dir=tmp_path / "run", run_fastqc=False, run_fastp=False
            ),
...
def test_wig_step_attributes_tolerate_tpp_comma_separators():
    rows = parse_tpp_wig_lines(["variableStep chrom=ref, replicon=1", "10 4"])

    assert rows == [InsertionCount("ref", 10, 4.0)]
